...
export class CounterEffects {
  ...
  // when we are told to load the counter state, check local storage, if it's there,
  // dispatch a document with that data, if it isn't, do nothing
  loadCounterState$ = createEffect(() => {
    return this.actions$.pipe(
      ...
      filter((storedValue) => storedValue !== null), // stop here if it's null - we'll stick with initialState => string // eslint-disable-next-line @typescript-eslint/no-non-null-assertion
